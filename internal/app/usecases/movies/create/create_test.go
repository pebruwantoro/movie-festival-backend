	...
	cases := []testCase{
		{
			name: "success",
			mock: func(mock testMock) {
				mock.mm.EXPECT().CreateMovie(gomock.Any(), gomock.Any()).Return(entities.Movie{}, nil)
			},
			Request:     req,
			ExpectedErr: nil,
		},
		{
			name: "error",
			mock: func(mock testMock) {
				mock.mm.EXPECT().CreateMovie(gomock.Any(), gomock.Any()).Return(entities.Movie{}, errors.New("got an error"))
			},
			Request:     req,
