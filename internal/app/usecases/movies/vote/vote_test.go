...
type testCase struct {
	...
	Request     VoteMovieRequest
	ExpectedErr error
}

func TestExecute(t *testing.T) {
	req := VoteMovieRequest{
		UserUUID:  "uuid-user",
		MovieUUID: "uuid-movie",
	...
	cases := []testCase{
		{
			name: "success",
			mock: func(mock testMock) {
				mock.vm.EXPECT().CreateVoter(gomock.Any(), gomock.Any()).Return(entities.Voters{}, nil)
			},
			Request:     req,
			ExpectedErr: nil,
		},
		{
			name: "error",
			mock: func(mock testMock) {
				mock.vm.EXPECT().CreateVoter(gomock.Any(), gomock.Any()).Return(entities.Voters{}, errors.New("got an error"))
			},
			Request:     req,
